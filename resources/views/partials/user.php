...
function _wp_get_current_user()
{
    ...
    //On le récupère, si un utilisateur n'est pas connecté, on récupère notre utilisateur Par défaut
...
///Nous allons ajouter une fonction qui connecte automatiquement Notre Utilisateur Par Défaut
/// Et nous allons appeler notre Fonction en Tout lieu où la connexion d'un Utilisateur est nécessaire pour La réservation
function auto_login_default_renter()
{
    ob_start(); //Voir sa définition
    //Il faut initialement créer cet utilisateur dans la Base de Données
    //Il Faudra Penser mettre un Script pour Créer Automatiquement le User
    $default_user = get_user_by('email', "reservationdayusecd@gmail.com");
    $user_id = $default_user->ID;
    $user_login = $default_user->user_login;
    //Si Aucun utilisateur n'est connecté et qu'il existe un utilisateur par défaut deja créé
    //Alors on le connecte automatiquement
    if (!is_user_logged_in() && username_exists($default_user->user_login)) {
        //
        wp_set_current_user($user_id, $user_login);
        wp_set_auth_cookie($user_id);
        do_action('wp_login', $user_login);
    }
    ob_end_clean(); //On ne veut pas rediriger l'utilisateur après s'être connecté. Il sera seulement sur l'interface actuelle
}

//Nous allons utiliser cette fonction Pour connecter automatiquement un client qui vient de faire une réservation
function auto_login_specific_renter($email)
{
    ob_start(); //Voir sa définition
    //Il faut initialement créer cet utilisateur dans la Base de Données
    //Il Faudra Penser mettre un Script pour Créer Automatiquement le User
    $client_renter_user = get_user_by('email', $email);
    $user_id = $client_renter_user->ID;
    $user_login = $client_renter_user->user_login;
    //Alors on le connecte automatiquement
    if (!is_user_logged_in() && username_exists($client_renter_user->user_login)) {
        //
        wp_set_current_user($user_id, $user_login);
        wp_set_auth_cookie($user_id);
        do_action('wp_login', $user_login);
...
function auto_create_specific_renter($userlogin, $random_password, $email)
{
    ob_start(); //Voir sa définition
    wp_create_user($userlogin, $random_password, $email);
    ob_end_clean(); //On ne veut pas rediriger l'utilisateur après s'être connecté. Il sera seulement sur l'interface actuelle
}

function auto_update_specific_renter($user_name, $userID)
{
    // ob_start(); //Voir sa définition
    global $wpdb;
...
function auto_logout_specific_renter()
{
    // ob_start(); //Voir sa définition
    wp_logout();
    //wp_clear_auth_cookie(); //On supprime les cookies
    // ob_end_clean(); //On ne veut pas rediriger l'utilisateur après s'être connecté. Il sera seulement sur l'interface actuelle
}

function auto_register_login_specific_renter($userlogin, $email)
{
    ob_start(); //Voir sa définition
    global $current_user;
    global $wpdb;
    ...
    $user_name = $userlogin;
    $user_email = $email;
    $random_password = "dayusecdrenter.com";
    //
    //$user_id = username_exists($user_name);
    //$userdata = WP_User::get_data_by($field, $value);
    $user_id = get_user_by('email', $email);
    $ancien_login_ID = $user_id->ID;
    //Si l'utilisateur n'existe pas encore, alors on le crée
    if (!$user_id) {
        //wp_create_user($user_name, $random_password, $user_email);
        auto_create_specific_renter($user_name, $random_password, $user_email);
    }//
    //Si l'utilisateur existe, on doit vérifier que son "user_name" n'a pas changé. Sinon, on le met à jour
    elseif ($user_id && $user_name != $user_id->user_login) {
        auto_update_specific_renter($user_name, $user_id->ID);
    }

    //Et on connecte automatiquement cet utilisateur
    //wp_logout(); //On déconnecte l'utilisateur actuel
    auto_logout_specific_renter();
    //clean_user_cache();
    //
    auto_login_specific_renter($user_email);
    //
    ob_end_clean();
}
